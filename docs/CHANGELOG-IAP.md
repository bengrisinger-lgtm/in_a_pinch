...
- **SKU 500 `42501` on consumer catalog (00037+ logs):** §1 columns present so startup migrate skipped — **no owner GRANT backfill** on `t_*` tables normalized by console → runtime **SELECT** still **42501**. Fix: `syncQuoteStoreRuntimeGrants` on every startup (admin pool); guest `GET /inventory/skus` read-only (no `expireStaleHolds`); staff-only maintenance/hold expiry.
- **SKU 500 `42501` (prior):** guest path ran `expireStaleHolds` UPDATEs against owner-owned tables. Mitigated by staff-only expiry + 42501 swallow in `expireStaleHolds`.
